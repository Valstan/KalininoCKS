...
**Status:** LIVE (народный визуальный образ Калинино на проде, 04.08)
**Updated:** 2026-08-04
**Branch:** main
...
- **PR #16 → prod:** squash merge `0f42b2d`; auto-deploy run `30936564846` — success за 2m56s (guard, standalone build, ship/restart, smoke `/`, `/news`, `/admin`). Внешний browser QA: главная содержит новый hero и эмблему; `/news` — 29 карточек и 6 чипов; `/news/category/novosti` — active chip + 1 карточка; первый пост — обложка + 4 элемента галереи; console errors = 0.
...
1. Владельцу на решение: долить 2 фото поста 1044; правка рубрик/текстов через /admin; порядок для свежих постов (повторный запуск vk-fetch + vk-import).
2. Перенос lint/typecheck в CI — ждём ответа Мозга (#104).
3. Почистить старый `NEXT_PUBLIC_SERVER_URL` (калинино-цкс.рф) в `/etc/kalinino/kalinino.env` на проде.
4. Drizzle-снапшот G192 — при починке локального Postgres (долг).
